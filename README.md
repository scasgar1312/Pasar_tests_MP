...
6. COMPILAR: ruta a un script que compile el proyecto que ha copiado.
	1. Manera manual (recomendada):
		...
		4. Copie los dos primeros comandos de compilación de NetBeans (son, generalmente, un «cd» y una llamada a make)
	...
	2. Usando el archivo de ejemplo. **Nota:** puede no estar actualizado

3. COMPILAR_MAIN: esta es una variable que sigue existiendo por motivos de compatibilidad, pero que debe tener el valor de la anterior.
...
10. Escribir en un archivo cómo debe cerrar el programa la función main() (esto previene al programa por si, en un futuro, tuviéramos que cambiar el cierre del main()). Ejemplo:
...
11. Apuntar la ruta absoluta al archivo resultante de la compilación (el ejecutable). Ejemplo:
...
**Nota importante**: si aparece un mensaje de «make[2]: 'dist/Debug/GNU-Linux/ejecutable' está actualizado.». Entonces, quiere decir que NetBeans ha detectado que no ha habido ningún cambio en el proyecto, así que no ha vuelto a compilarlo. Por tanto, deberá hacer un cambio cualquiera (añadir o quitar un comentario sin importancia) para obligar a que NetBeans detecte que el proyecto no es el mismo y lo compile.
  
11. Modificar el archivo main.cpp de la copia del proyecto, vacíelo menos los `include` y el `using namespace std;`, es decir, borre la función `main` completamente.
  
12. Copie el código que debemos poner en el main.cpp para pasar los tests de integridad. Recomendación: ejecute
```
...
Descargue la versión más reciente [aquí](https://github.com/scasgar1312/Pasar_tests_MP/releases) (será un archivo comprimido que deberá descomprimir).
...
Por cortesía de Manuel RA, el TestReport.md ya corregido está en el siguiente enlace: [https://0.jaegers.net/?7c3389254da762fe#7TtR4iVP2safnyfwA7iUPPE9ZAri56DHhKrzfCV26A2N](https://0.jaegers.net/?7c3389254da762fe#7TtR4iVP2safnyfwA7iUPPE9ZAri56DHhKrzfCV26A2N)
